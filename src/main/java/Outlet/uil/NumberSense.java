...
public class NumberSense extends HttpServlet{
    public static Template template;
    public static final short cid = 4;
    ...
    public static final String[] KEY = {"a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a",
            ...
            "a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a","a",
            "a","a","a","a","a","a"}; // Multiple choice key
    ...
    public static final int MC_NUM_PROBLEMS = 80;
    ...
    public static final long MC_TIME = 10*60*1000;

    /**
     * Timing constants
     */
    private static String opensString = "08/14/2020 18:00:00";
    private static String closesString = "08/16/2020 18:00:00";
    private static Countdown opens;
    private static Countdown closes;
    public static final String MC_TIME_TEXT = "10 minutes";


    /***
     * Text constants like names and descriptions.
     */
    public static final String NAME = "UIL Number Sense";
    public static final String WHAT_IT_IS = "A fast-paced ten-minute 80-question mental math test covering all levels of high school math.";
    ...
    public static void initialize() {
        ...
        template = new Template(NAME, WHAT_IT_IS, RULES, PRACTICE, mc, frq, opens, closes, cid, new SortNumberSenseTeams());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("updatePage")) {  // Reloads the columns portion of the page
            ...
            if(template.getStatus() == 1) {
                NumberSenseEntry temp = (NumberSenseEntry) user.team.comps.get(cid);
...
class NumberSenseEntry extends UILEntry{
    public final short tid;
    private static Gson gson = new Gson();

    public NumberSenseEntry(ResultSet rs) throws SQLException {
    ...
    public NumberSenseEntry(short tid, HashMap<Short, MCSubmission> mc) {
        super(tid);
        this.tid = tid; this.mc = mc;
    }

    /**
     * Loads an EXISTING NumberSenseEntry from the database. Does not create a new one.
     * @param tid
     */
    public static NumberSenseEntry loadEntry(short tid) throws SQLException {
        Connection conn = Conn.getConnection();
        PreparedStatement stmt = conn.prepareStatement("SELECT * FROM `c"+NumberSense.cid+"` WHERE tid=?");
        stmt.setShort(1,tid);
        ResultSet rs = stmt.executeQuery();
        if(rs.next()) {
            return new NumberSenseEntry(rs);
        }
        return null;
    }
     ...
     * @throws SQLException
     */
    public static NumberSenseEntry signup(short tid) throws SQLException {
        NumberSenseEntry newEntry = new NumberSenseEntry(tid, new HashMap<>());

        Connection conn = Conn.getConnection();
        PreparedStatement stmt = conn.prepareStatement("INSERT INTO `c" + NumberSense.cid + "` VALUES (?, ?)");
    ...
    public void update(){
        Connection conn = Conn.getConnection();
        try {
            PreparedStatement stmt = conn.prepareStatement("UPDATE `c"+NumberSense.cid+"` SET mc=? WHERE tid=?");
    ...
    /**
     * Returns a sorted array of Teams which are signed up for NumberSenseEntry.
     * @return
     */
    public static ArrayList<Team> getAllEntries(){
        ...
        for(Team t: allTeams) {
            if(t.comps.keySet().contains(NumberSense.cid)){
                teams.add(t);
            }
        }
        return teams;
    ...
    public long beginMC(short uid) {
        ...
        mc.put(uid, new MCSubmission(NumberSense.MC_NUM_PROBLEMS,now));
        return now;
    }
    ...
    public short[] scoreMC(short uid, String[] answers){
        ...
        entry.scoringReport = NumberSense.template.mcTest.score(answers);
...
/**
 * Sorts teams that belong to the NumberSense competition by their NumberSense score
 */
class SortNumberSenseTeams extends SortUILTeams implements Comparator<Team>
{
    public int compare(Team a, Team b) {
        return b.comps.get(NumberSense.cid).getScore()-a.comps.get(NumberSense.cid).getScore();
    }
}